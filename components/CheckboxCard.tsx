...
export type CheckboxCardState = 'Default' | 'Hover' | 'Selected' | 'Disabled';
...
export const CheckboxCard: React.FC<CheckboxCardProps> = ({
  ...
  const effectiveState: CheckboxCardState = isDisabled 
    ? 'Disabled' 
  ...
  const getCardClasses = () => {
    ...
    if (isSelected) {
      return `${baseClasses} border-[#155eef] ${effectiveState === 'Hover' ? 'shadow-sm' : ''}`;
    }

    if (effectiveState === 'Hover') {
      return `${baseClasses} border-[#b2ccff] shadow-[0px_1px_2px_0px_rgba(16,24,40,0.05)] cursor-pointer`;
    }
